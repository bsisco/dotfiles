...
local function worker(format, warg)
    ...
    weather["{heatinx}"] = -- Heat Index
        string.match(ws, "Heat Index:[%s]([%-]?[%d%.]+).*[%c]") or weather["{heatinx}"]
    ...
    if weather["{heatinx}"] ~= "N/A" then
        weather["{heatinx}"] = tonumber(weather["{heatinx}"])
        weather["{heatinx}"] = math.ceil((weather["{heatinx}"] -32) * 5/9)
    end
